...
export function evaluateDiscoveryPayload(
  ...
  for (const [index, stock] of frontStocks.entries()) {
    ...
    if (PRICE_ONLY_EXACT_PATTERN.test(hook) || PRICE_ONLY_PATTERN.test(hook) || NO_MATERIAL_PATTERN.test(hook)) {
      add("hook.price_only", `앞단 ${index + 1}번 카드 '${name}'가 가격-only 또는 재료 없음 문장입니다.`, hook);
    }
